$(function() {
  const X_VALUES = [-5.0, -4.0, -3.0, -2.0, -1.0, 0.0, 1.0, 2.0, 3.0];
  const Y_MIN = -5;
  const Y_MAX = 5;
  ...
  $('.input-form__radio').on('click', function (event){
    xval = $(this).val();
    $(this).addClass('input-form__radio_clicked');
    $('.input-form__radio').not(this).removeClass('input-form__radio_clicked');
  })
  ...
  function validateX(){
      if(xval !== undefined){
        ...
        $('.input-form__info').text('Select X value!')
        return false;
      }
  }
  function validateY() {
    ...
    if (isNumeric(numY) && numY >= Y_MIN && numY <= Y_MAX)
    {
      ...
      $('.input-form__info').text('Enter a Y value between -5 and 5!')
      return false;
    }
  }
...
//Here for X i need to correct function 3 line from here down
